...
/** 上传单个图片
 * @param string|上传的文件名称 $files 上传的文件名称
 * @param string|文件上传指定目录，在Uploads下 $filePath 文件上传指定目录，在Uploads下
 * @param string $savepath
 * @param int $size
 * @param string $prefix
 * @return String 返回图片信息 PREFIX
 * @internal param 指定生成的图片文件夹格式，如2016 $filename /03/30
 */
 function  uploadOneImage($files='',$filePath='',$savepath='Y/m/d',$size=512000 , $prefix=''){
     $config = array(
         'maxSize'    =>    $size,
         'rootPath'   =>    './Uploads/'.$filePath.'/',
         'savePath'   =>    '',
         'saveName'   =>    $prefix?$prefix:array('uniqid',''),
         'exts'       =>    array('jpg', 'gif', 'png', 'jpeg'),
         'autoSub'    =>    true,
         'subName'    =>    array('date',$savepath),
         'replace'    =>    true,
     );
    ...
    //  dump($info);
     if(!$info) {   // 上传错误提示错误信息
        ...
        return array(1,'Uploads/'.$filePath.'/'.$info['savepath'].$info['savename']);
     }
 }
 ...
 * @return bool|string
 * @internal param string $path 保存图片的主文件夹
 */
 ...
 * @internal param string $key 配置键名
 */
 ...
 * @param array|string $array $array 搜索条件
...
/** 解密方式
 * @param  $str   传入字符串
 * @param string|私钥字串 $privatekey 私钥字串
 * @param 公钥字串，默认为公共 $publickey 公钥字串，默认为公共 config 配置文件内 DATA_AUTH_KEY
 * @return String
 */
 ...
 * @param array $data
 */
function myAjaxReturn($code,$msg,$data=array()){
    echo $data!==array() ?
...
function sendMail($to, $title, $obj, $ap ,$name)
{
    ...
    try {
        ...
        $mail->Username = 'csk_shawn@163.com';                 // SMTP username
        $mail->Password = 'mingtianjian1111';                           // SMTP password
        //$mail->SMTPSecure = 'ssl';                            // Enable TLS encryption, `ssl` also accepted
        $mail->Port = 25;                                    // TCP port to connect to
        //Recipients
        $mail->setFrom('csk_shawn@163.com', 'CityEye');
        $mail->addAddress($to);     // Add a recipient
        $mail->addReplyTo($ap, $name);
        //Content
        $mail->isHTML(true);                                  // Set email format to HTML
        $mail->Subject = $title;
        $mail->Body = $obj;
        $mail->AltBody = 'This is email form cityEye';
        $mail->send();
        return true;
